...
This is the second project in the series, and the first to introduce two related entities and the Collections framework. Its central challenge — and the main lesson it demonstrates — is coordinating two independent repositories from a single service method (`allocateRoom`, `vacateStudent`) without letting either repository know the other exists, keeping the cross-entity business rule in exactly one place.
